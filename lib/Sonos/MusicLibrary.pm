...
sub children( $self, $parent ) {
    ...
    @items = sort { $a->originalTrackNumber() <=> $b->originalTrackNumber() || fc( $a->title() ) cmp fc( $b->title() ) }
      @items;
    return @items;
}
...
sub TO_JSON( $self, $qf ) {
    ...
    if ($msearch) {
        ...
        return { "items" => [ map { $_->TO_JSON() } @elements ] };
    }
...
sub addItemsOnly( $self, @items ) {
    ...
    for (@items) {
        ...
        $self->{_tree}->{$parentid} = []
          unless defined $self->{_tree}->{$parentid};
        my $itemlist = $self->{_tree}->{$parentid};
        warn "Item with id $id already seems to be a parent of $parentid"
          if grep { $_ eq $id } @$itemlist;
