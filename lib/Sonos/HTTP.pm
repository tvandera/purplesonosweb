...
sub validateRequest( $self, $r ) {
    ...
    if ( exists $qf{zone} ) {
        ...
        return $self->sendError( $r, 404, "No such player: $zone" )
          unless $player;
    }
    ...
    if ( exists $qf{mpath} ) {
        ...
        return $self->sendError( $r, 404, "No such music item: $path" )
          unless $item;
    }

    if ( exists $qf{queue} ) {
        my $path = decode( "UTF-8", $qf{queue} );
        return $self->sendError( $r, 404, "queue=$path requires a zone=" )
          unless $player;

        my $item = $player->queue()->item($path);
        return $self->sendError( $r, 404, "No such queue item: $path" )
          unless $item;
    }
...
sub sendAlbumartResponse( $self, $r ) {
    ...
    my ( $sha, $mime_type, $blob, $filename ) =
      $self->system()->albumArtCache()->get($uri);
